...
if __name__ == '__main__':
    ...
    model_saving_dir = '/Users/winslowfan/Documents/Lectures/Sheffield/Final Dissertation/ReinforcementLearning/TD3_preTrained/'
    now = datetime.now()
    ...
    max_train_steps = 6e6  # Maximum number of training steps
    save_freq = int(3e5)
    ...
    while total_steps < max_train_steps:
        ...
        while not done:
            ...
            if total_steps % save_freq == 0:
                actor_path = model_saving_dir + 'TD3_actor_{}_{}.pth'.format(datetime.now().strftime("%m-%d-%Y_%H-%M-%S"), i_episode)
                critic_path = model_saving_dir + 'TD3_critic_{}_{}.pth'.format(datetime.now().strftime("%m-%d-%Y_%H-%M-%S"), i_episode)
                actor_target_path = model_saving_dir + 'TD3_actor_target_{}_{}.pth'.format(datetime.now().strftime("%m-%d-%Y_%H-%M-%S"), i_episode)
                critic_target_path = model_saving_dir + 'TD3_critic_target_{}_{}.pth'.format(datetime.now().strftime("%m-%d-%Y_%H-%M-%S"), i_episode)
                ...
                print('model saved for {}'.format(i_episode))

            if lr_decay and total_steps % lr_decay_freq == 0:
                agent.actor_optimizer.param_groups[0]['lr'] *= lr_decay_rate
                agent.critic_optimizer.param_groups[0]['lr'] *= lr_decay_rate

            total_steps += 1

    run_time = datetime.now() - now
    print("Running time is {}".format(run_time))
